...
while utime.ticks_diff(utime.ticks_us(), start_time) <= TEST_TIME_US:
    ...
    if utime.ticks_diff(current_time, next_sampling_time) >= 0:
        # INSIDE OF THE IF STATEMENT
        # WRITE YOUR CODE HERE to update the next sampling time.
        next_sampling_time = utime.ticks_add(next_sampling_time, SAMPLING_PERIOD_US)

        # WRITE YOUR CODE HERE to read the motor velocity, using the encoder.
        vel = enc.get_velocity_rad()

        # WRITE YOUR CODE HERE to put the data in the queues.
        # Example: time_queue.put(t)
        time_queue.put(current_time)
        velocity_queue.put(vel)
...
# 4. UNCOMMENT THE CODE BELOW to print the data in CSV format.
